...
<rss version="2.0" xmlns:content="http://purl.org/rss/1.0/modules/content/"<?php foreach ($namespaces as $n) {
        echo ' ' . $n;
    } ?>>
    <channel>
        ...
        @foreach ($items as $item)
            <item>
                ...
                @if (!empty($item['enclosure']))
                    <enclosure @foreach ($item['enclosure'] as $k => $v) {!! $k . '="' . $v . '" ' !!} @endforeach />
                @endif
                @if (!empty($item['media:content']))
                    <media:content @foreach ($item['media:content'] as $k => $v) {!! $k . '="' . $v . '" ' !!} @endforeach />
                @endif
                @if (!empty($item['media:thumbnail']))
                    <media:thumbnail
                        @foreach ($item['media:thumbnail'] as $k => $v) {!! $k . '="' . $v . '" ' !!} @endforeach />
                @endif
